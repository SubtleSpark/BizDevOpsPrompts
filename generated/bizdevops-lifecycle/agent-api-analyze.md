...
#### 请求响应结构表示规范:
- ✅ **使用JSON Schema格式**: 清晰表示数据类型和结构层次
- ✅ **标明必填性**: required/optional明确标识
- ✅ **包含约束条件**: 枚举值、数值范围、字符串格式等
- ✅ **嵌套结构展示**: 对象和数组的完整结构
- ✅ **业务语义说明**: 每个字段的业务含义和使用场景

**约束标识规范**:
```
类型 (必填性, 约束条件) - 业务描述
例如: "string (required, length<=50)"
例如: "enum ['active', 'inactive'] (required)"
例如: "number (optional, >0, 分为单位)"
```
...
**质量保证**: 文档生成后，建议使用 `agent-doc-quality-checker.md` 进行质量检查和改进建议。